...
import { BookOpen, Lightbulb, Upload, FileText, Archive, MessageSquare, Cat, Layers3, PenLine, Highlighter, Scan, StickyNote, Sparkles, Download, Settings2, X, ChevronLeft, ChevronRight, Minus, Plus, Check, Trash2, LoaderCircle, FolderOpen, PanelRightClose, PanelRightOpen } from 'lucide-react';
import type { Chat, Crop, Note, Paper, Selection } from '@/lib/reader-types';
import { appendCrop, cropsSelection, replaceCrop, selectionCrops, selectionLabel } from '@/lib/crops';
...
import ReadAloud, { SpeechStatus } from '@/components/read-aloud';
...
import { UpdateNotice } from '@/components/updates-panel';
...
const documentBaseName = (name: string) => name.replace(/\.(?:pdf|pptx?|odp)$/i, '');
...
export default function Home() {
  ...
  const [drawAfterCrop, setDrawAfterCrop] = useState(false);
  const [editingCrop, setEditingCrop] = useState<{ crop: Crop; index: number } | null>(null);
  ...
  function choosePaper(item: Paper) {
    cancelQuestion(); setEditingCrop(null); setView('reader'); setPaper(item); setScope('paper'); setSelection(null); setDraft(null); setQuestion(''); setActiveNote(null); setLibrary(false); setPageCount(0); setZoom(1); setError('');
  }
  ...
  async function openFile(file?: File) {
    ...
    try {
      ...
      const isSlides = /\.(?:pptx?|odp)$/i.test(file.name);
      if (!isPdf && !(isSlides && file.type === 'application/pdf')) throw new Error(isSlides ? 'Use Open document in the installed app to convert PowerPoint slides for reading.' : 'Choose a PDF, PowerPoint deck, or a saved Paper Reader ZIP bundle.');
      if (file.size > 50 * 1024 * 1024) throw new Error('Choose a PDF or converted slide deck smaller than 50 MB.');
    ...
    } catch (e) { setError(e instanceof Error && /password/i.test(e.message) ? 'This PDF is password protected. Open an unlocked copy.' : e instanceof Error ? e.message : 'This document could not be opened.'); }
    finally { setOpening(false); if (input.current) input.current.value = ''; }
  }
  async function browse() {
    if (!window.paperReader) { input.current?.click(); return; }
    try { const file = await window.paperReader.openPdf(); if (file) await openFile(new File([new Uint8Array(file.bytes)], file.name, { type: file.convertedSlides ? 'application/pdf' : file.name.toLowerCase().endsWith('.zip') ? 'application/zip' : 'application/pdf' })); }
  ...
  function pickSelection(value: Selection) {
    if (busy) return;
    try {
      const next = appendCrop(scope === 'selection' ? selection : null, value);
      setSelection(next);
      if (value.kind === 'area' && drawAfterCrop) { const crops = selectionCrops(next); setEditingCrop({ crop: crops[crops.length - 1], index: crops.length - 1 }); }
      setScope('selection'); setDraft(null); setTab('ask'); setPanel(true); setActiveNote(null); setError('');
    } catch (e) { setError(e instanceof Error ? e.message : 'Could not add this crop.'); }
  }
  function removeCrop(index: number) {
    if (busy) return;
    const next = cropsSelection(selectionCrops(selection).filter((_, i) => i !== index));
    setSelection(next); setScope(next ? 'selection' : 'paper'); setDraft(null); setError('');
  }
  function saveCropDrawing(crop: Crop) {
    if (!selection || !editingCrop || busy) return;
    setSelection(replaceCrop(selection, editingCrop.index, crop));
    setDraft(null); setEditingCrop(null); setError('');
  }
  ...
  async function ask(override?: string) {
    ...
    try {
      ...
      if (request.current?.id !== id || currentPaper.current?.id !== source.id) return;
      const context = { question: asked, model: chosenModel, sourceKind: /\.(?:pptx?|odp)$/i.test(source.name) ? 'slides' as const : 'pdf' as const, pdf: { filename: documentBaseName(source.name.slice(0, 240)) + '.pdf', data } };
      const crops = selectionCrops(selected);
      if (crops.some(crop => !crop.image)) throw new Error('These saved notes have crop locations only. Crop the areas again before sending images to ChatGPT.');
      const payload: AskPayload = selected.kind === 'paper' ? { ...context, scope: 'paper' } : {
        ...context, scope: 'selection', text: selected.text, page: selected.page,
        ...(crops.length ? { crops: crops.map(crop => ({ page: crop.page, image: crop.image! })) } : {}),
      };
  ...
  async function catQuiz(token: string) {
    ...
    try {
      ...
      if (request.current?.id !== id || currentPaper.current?.id !== source.id) return;
      const result = await bridge.ask({ id, scope: 'paper', model: chosenModel, sourceKind: /\.(?:pptx?|odp)$/i.test(source.name) ? 'slides' : 'pdf', question: quizPrompt(source.page, previous), pdf: { filename: documentBaseName(source.name.slice(0, 240)) + '.pdf', data } });
      if (request.current?.id !== id || currentPaper.current?.id !== source.id) return;
      if (result.error) throw new Error(result.error);
  ...
  async function generateFlashcards(count: number) {
    ...
    try {
      ...
      const batches = flashcardBatches({ count, pages, isActive: () => request.current?.id === id && currentPaper.current?.id === source.id,
        send: question => bridge.ask({ id, scope: 'paper', model: chosenModel, sourceKind: /\.(?:pptx?|odp)$/i.test(source.name) ? 'slides' : 'pdf', question, pdf: { filename: documentBaseName(source.name.slice(0, 240)) + '.pdf', data } }),
      });
  ...
  async function downloadBundle() {
    ...
    try {
      ...
      const name = documentBaseName(source.name) + ' - document and chats.zip';
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      const bytes = await exportPdf(paper); const name = documentBaseName(paper.name) + ' - annotated.pdf';
  ...
  return <main className="app-shell" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); void openFile(e.dataTransfer.files[0]); }}>
    ...
    {editingCrop && <CropEditor crop={editingCrop.crop} index={editingCrop.index} onSave={saveCropDrawing} onClose={() => setEditingCrop(null)}/>}
