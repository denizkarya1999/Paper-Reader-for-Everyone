...
export async function importBundle(bytes: Uint8Array): Promise<{ paper: Paper; chats: Chat[] }> {
  ...
  if (parsed.data.chats.some(chat => chat.selection.page > document.getPageCount())) throw new Error('A chat in this bundle points to a page that is missing from its PDF.');
