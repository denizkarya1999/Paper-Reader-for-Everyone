...
const chatSchema = z.object({
  ...
  selection: z.object({ page: z.number().int().positive().max(100000), kind: z.enum(['text', 'area', 'paper']), text: z.string().max(30000), rects: z.array(rect).min(1).max(500), image: z.string().max(5_000_000).regex(/^data:image\/png;base64,[A-Za-z0-9+/]+=*$/).optional() }),
});
...
export function historyHtml(paperName: string, chats: z.infer<typeof chatSchema>[]): string {
  const entries = chats.map(chat => '<article><header>' + escape(chat.selection.kind === 'paper' ? 'Whole paper' : 'PDF page ' + chat.selection.page) + ' · ' + escape(chat.model) + ' · ' + escape(chat.createdAt) + ' · ' + escape(chat.status) + '</header><h2>' + escape(chat.question) + '</h2>' + (chat.selection.text ? '<blockquote>' + escape(chat.selection.text) + '</blockquote>' : '') + (chat.selection.image ? '<img alt="Selected PDF crop" src="' + escape(chat.selection.image) + '">' : '') + '<pre>' + escape(chat.answer || chat.error || 'No answer was saved.') + '</pre>' + (chat.flashcards || []).map((card, index) => '<section><h3>Flashcard ' + (index + 1) + ' · PDF page ' + card.page + '</h3><h4>' + escape(card.question) + '</h4><pre>' + escape(card.answer) + '</pre></section>').join('') + '</article>').join('\n');
...
export async function importBundle(bytes: Uint8Array): Promise<{ paper: Paper; chats: Chat[] }> {
  ...
  if (parsed.data.chats.some(chat => chat.selection.page > document.getPageCount() || chat.flashcards?.some(card => card.page > document.getPageCount()))) throw new Error('A chat or flashcard in this bundle points to a page that is missing from its PDF.');
