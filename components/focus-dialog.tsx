...
export default function FocusDialog({ onClose, embedded = false }: { onClose: () => void; embedded?: boolean }) {
  ...
  const content = <section className={embedded ? 'settings-form focus-modal' : 'modal focus-modal'} role={embedded ? undefined : 'dialog'} aria-modal={embedded ? undefined : true} aria-labelledby="focus-title" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape' && !saving && !embedded) onClose(); }}>
    {!embedded && <><button className="modal-close" aria-label="Close cat settings" onClick={onClose} disabled={saving}><X size={20}/></button>
    ...
    <form onSubmit={e => { e.preventDefault(); void save(); }}><fieldset disabled={loading || saving}>
      <label className="remember-key"><input autoFocus={!embedded} type="checkbox" checked={value.enabled} onChange={e => setValue({ ...value, enabled: e.target.checked })}/><span>Show my cat</span></label>
      ...
      <div className="modal-actions">{!embedded && <button type="button" className="text-button" onClick={onClose}>Cancel</button>}<button className="button primary" type="submit">{saving ? 'Saving…' : 'Save cat settings'}</button></div>
    </fieldset></form>
  </section>;
  return embedded ? content : <div className="modal-backdrop" onClick={() => { if (!saving) onClose(); }}>{content}</div>;
}
