...
type Props = { connection: ConnectionState; onClose: () => void; onChange: (state: ConnectionState) => void; embedded?: boolean };
export default function ConnectionDialog({ connection, onClose, onChange, embedded = false }: Props) {
  ...
  const content = <section className={embedded ? 'settings-form connection-modal' : 'modal connection-modal'} role={embedded ? undefined : 'dialog'} aria-modal={embedded ? undefined : true} aria-labelledby="connection-title" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape' && !busy && !embedded) onClose(); }}>
      {!embedded && <><button className="modal-close" aria-label="Close connection settings" disabled={busy} onClick={onClose}><X size={20}/></button>
      ...
      <form onSubmit={e => { e.preventDefault(); void save(); }}>
        ...
        <input autoFocus={!embedded} id="api-key" type="password" autoComplete="off" spellCheck={false} placeholder={connection.hasKey ? 'Leave blank to keep your current key' : 'sk-…'} value={key} maxLength={512} disabled={busy} onChange={e => setKey(e.target.value)}/>
      ...
      <div className="connection-footer">{!embedded && <button className="text-button" disabled={busy} onClick={onClose}>{connection.hasKey ? 'Cancel' : 'Read without AI for now'}</button>}{(connection.hasKey || connection.saved) && <button className="text-button remove-key" disabled={busy} onClick={() => void remove()}><Trash2 size={13}/>Remove key</button>}</div>
    </section>;
  return embedded ? content : <div className="modal-backdrop" onClick={() => { if (!busy) onClose(); }}>{content}</div>;
}
